...
metadata:
  ...
  modified: "Project integration guidance added for Markdown Viewer"
---
...
This project also has a `ui-guidelines` skill. Read and apply it before
implementation: this skill owns intentional visual direction, while
`ui-guidelines` owns compatibility with the established tokens, CSS architecture,
components, themes, and interaction conventions. Routine UI work preserves the
existing visual language. Change those conventions only for an explicitly
requested redesign, and update `ui-guidelines` in the same change.
